...
const RunwayBar = () => {
  ...
  const [commitProduct, setCommitProduct] = useState<{ id: string; name: string; logoEmoji: string } | null>(null);
  ...
  const handleProductSelect = (p: typeof products[0], slotIndex: number) => {
    setCommitProduct({ id: p.id, name: p.name, logoEmoji: p.logoEmoji });
    setOpenSlot(null);
    setShowCommitModal(true);
  };

  const handleCommitConfirm = (targetDate: Date, remindersEnabled: boolean) => {
    if (!commitProduct) return;
    addToRunway(commitProduct, targetDate, remindersEnabled);
    toast.success(`${commitProduct.name} added to your Runway! 🚀`);
    setCommitProduct(null);
  };
  ...
  return (
    <>
                            ...
                            filteredAvailable
                              ...
                              .map((p) => (
                                <button
                                  ...
                                  onClick={() => handleProductSelect(p, i)}
                                >
      ...
      <RunwayCommitModal
        ...
        productName={commitProduct?.name ?? ""}
